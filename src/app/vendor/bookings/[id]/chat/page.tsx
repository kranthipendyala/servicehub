...
export default function VendorChatPage() {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      const res = await getVendorChatMessages(bookingId);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setSending(true);
    try {
      await sendVendorChatMessage(bookingId, newMessage);
      setNewMessage("");
      fetchMessages();
    } catch {
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-white border border-gray-200 rounded-t-xl">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">Chat with customer</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto bg-gray-50 border-x border-gray-200 px-4 py-4 space-y-3">
        {loading ? (
          ...
          messages.map((msg) => {
            const isVendor =
              msg.sender_role === "vendor" || msg.sender_role === "business_owner";
            return (
              <div
                key={msg.id}
