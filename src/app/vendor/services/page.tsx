    ...
    return (
      <div className={`flex items-center justify-between gap-4 p-4 rounded-xl border transition-all ${active ? "bg-white border-gray-200 hover:shadow-sm" : "bg-gray-50 border-gray-100 opacity-70"}`}>
        <div className="flex items-center gap-4 min-w-0 flex-1">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold flex-shrink-0 ${active ? "bg-emerald-100 text-emerald-700" : "bg-gray-200 text-gray-500"}`}>
        ...
        <div className="flex items-center gap-0.5 flex-shrink-0">
          ...
          <button onClick={() => handleToggleActive(service)} className={`p-2 rounded-lg transition-colors ${active ? "hover:bg-amber-50 text-amber-400 hover:text-amber-600" : "hover:bg-emerald-50 text-emerald-400 hover:text-emerald-600"}`} title={active ? "Deactivate" : "Activate"}>
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              {active ? (
    ...
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link
          ...
          className="px-4 py-2.5 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl transition-colors flex items-center gap-2 shadow-sm"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {services.length === 0 && Object.keys(grouped).length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center">
          ...
          <p className="text-sm text-gray-500 mb-6 max-w-sm mx-auto">
            First <Link href="/vendor/categories" className="text-emerald-600 font-medium hover:underline">select your categories</Link>, then add services under each category.
          </p>
          <div className="flex items-center justify-center gap-3">
            <Link href="/vendor/categories" className="px-4 py-2.5 text-sm font-medium text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-xl transition-colors">
              Select Categories
            </Link>
            <Link href="/vendor/services/new" className="px-4 py-2.5 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl transition-colors flex items-center gap-2">
            ...
            <div key={catId} className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
              ...
              <div className="p-3">
                {group.services.length === 0 ? (
                  <Link href={`/vendor/services/new?cat=${catId}`} className="flex items-center justify-center gap-2 py-8 text-sm text-gray-400 hover:text-emerald-600 transition-colors">
