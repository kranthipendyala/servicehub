        ...
        {NAV_ITEMS.map((item) => {
          ...
          if (item.accent) {
            return (
              <Link key={item.label} href={item.href} className="flex flex-col items-center justify-center -mt-5">
                <div className={`w-14 h-14 rounded-full flex items-center justify-center shadow-lg ${isActive ? "bg-emerald-600 shadow-emerald-500/30" : "bg-gradient-to-br from-emerald-500 to-teal-600 shadow-teal-500/30"}`}>
                  <svg className="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d={item.icon} />
                  </svg>
                </div>
                <span className="text-[10px] font-semibold text-emerald-600 mt-1">{item.label}</span>
              </Link>
            );
          }
          ...
          return (
            <Link key={item.label} href={item.href} className="flex flex-col items-center justify-center flex-1 py-2">
              <svg className={`w-6 h-6 ${isActive ? "text-emerald-600" : "text-gray-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={isActive ? 2.5 : 1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d={item.icon} />
              </svg>
              <span className={`text-[10px] mt-0.5 ${isActive ? "font-bold text-emerald-600" : "font-medium text-gray-400"}`}>{item.label}</span>
