      ...
      <div className="min-h-screen flex items-center justify-center bg-accent-200">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="text-center">
            <p className="text-sm font-semibold text-primary-700">Loading business...</p>
            <p className="text-xs text-primary-700/50 mt-0.5">Just a moment</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-accent-200">
        <div className="text-center max-w-sm mx-auto px-4">
          ...
          <p className="text-primary-700/70 text-sm mb-6 leading-relaxed">The business you&apos;re looking for doesn&apos;t exist or has been removed.</p>
          <Link
            href="/"
            className="btn-primary rounded-btn inline-flex items-center gap-2 px-6 py-3 text-sm font-semibold transition-all duration-200 ease-advia"
          >
  ...
  return (
    <div className="min-h-screen bg-accent-200 pb-24 lg:pb-0">
              ...
              {biz.categories && biz.categories.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3">
                  {biz.categories.map((cat: any) => (
                    <span
                      key={cat.id}
                      className="inline-flex items-center gap-1.5 text-xs font-semibold text-primary-700/70 bg-white px-3 py-1.5 rounded-btn"
                    >
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-4 text-sm">
                {biz.city_name && (
                  <span className="inline-flex items-center gap-1.5 text-primary-700/70">
                    ...
                    <span className="font-medium">{biz.city_name}{biz.state_name ? `, ${biz.state_name}` : ""}</span>
                  </span>
                )}
                {avgRating > 0 && (
                  <span className="inline-flex items-center gap-1.5">
                    ...
                    <span className="text-primary-700/70 text-xs font-medium">({totalReviews} reviews)</span>
                  </span>
                )}
              </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {biz.description && (
              <section className="bg-white rounded-card p-6 lg:p-8 hover:-translate-y-[2px] transition-all duration-200 ease-advia">
                ...
                <p className="text-primary-700/70 text-sm leading-relaxed">{biz.description}</p>
              </section>
            )}
            ...
            {biz.services_by_category && biz.services_by_category.length > 0 && (
              <section className="bg-white rounded-card p-6 lg:p-8 hover:-translate-y-[2px] transition-all duration-200 ease-advia">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <Link
                    href={`/book/${slug}`}
                    className="hidden sm:inline-flex items-center gap-1 text-sm font-semibold text-primary-600 hover:text-primary-700 transition-colors duration-200 ease-advia group"
                  >
                    Book now
                ...
                <div className="space-y-8">
                  {biz.services_by_category.map((group: any) => (
                    <div key={group.category_id}>
                      <h3 className="section-label-badge mb-3">{group.category_name}</h3>
                      <div className="space-y-2">
                        {group.services.map((svc: any) => (
                          <div
                            key={svc.id}
                            className="group flex items-center justify-between p-4 bg-accent-100 rounded-card border border-transparent hover:border-primary-200 transition-all duration-200 ease-advia"
                          >
                            <div className="min-w-0 flex-1">
                              <p className="text-sm font-semibold text-primary-700 group-hover:text-primary-600 transition-colors duration-200 ease-advia">{svc.name}</p>
                              {svc.duration_minutes > 0 && (
                                <p className="text-xs text-primary-700/50 mt-0.5 flex items-center gap-1">
                            ...
                            {Number(svc.base_price) > 0 && (
                              <span className="text-sm font-bold text-primary-700 flex-shrink-0 ml-3">
                ...
                <div className="sm:hidden mt-6 text-center">
                  <Link
                    href={`/book/${slug}`}
                    className="inline-flex items-center gap-1.5 text-sm font-semibold text-primary-600 hover:text-primary-700 transition-colors duration-200 ease-advia"
                  >
                    View all &amp; book now
            ...
            <div className="lg:sticky lg:top-24">
              <div className="bg-accent-200 rounded-card p-6 space-y-4">
                <div className="text-center">
                  ...
                  <p className="text-sm text-primary-700/70 mt-1">Schedule your service today and get expert help.</p>
                </div>

                <Link
                  href={`/book/${slug}`}
                  className="btn-primary rounded-btn flex items-center justify-center gap-2 w-full py-3.5 text-sm font-semibold transition-all duration-200 ease-advia"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" /></svg>
                  Book a Service
                </Link>
                {biz.phone && (
                  <button
                    onClick={() => setContactOpen(true)}
                    className="btn-secondary rounded-btn flex items-center justify-center gap-2 w-full py-3 text-sm font-semibold transition-all duration-200 ease-advia"
                  >
                    <svg className="w-4 h-4 text-primary-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                    Contact Business
                  </button>
                  ...
                  <div className="space-y-3 text-sm">
                    {biz.phone && (
                      <a href={`tel:${biz.phone}`} className="flex items-center gap-3 group">
                        <div className="w-10 h-10 rounded-card bg-accent-200 flex items-center justify-center flex-shrink-0 transition-all duration-200 ease-advia">
                          <svg className="w-4 h-4 text-primary-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                        </div>
                        <span className="text-primary-600 font-medium group-hover:underline">{biz.phone}</span>
                      </a>
                    )}
                    {biz.email && (
                      <a href={`mailto:${biz.email}`} className="flex items-center gap-3 group">
                        ...
                        <span className="text-primary-600 font-medium group-hover:underline truncate">{biz.email}</span>
                      </a>
                    )}
                    {biz.address && (
                      <div className="flex items-start gap-3">
                        ...
                        <p className="text-primary-700/70 leading-relaxed">{biz.address}</p>
                      </div>
                    )}
                    {biz.website && (
                      <a href={biz.website} target="_blank" rel="noopener" className="flex items-center gap-3 group">
                        ...
                        <span className="text-primary-600 font-medium group-hover:underline truncate">{biz.website}</span>
                      </a>
                    )}
                  </div>
                </div>
      ...
      {/* ══════════════════════════════════════════════════
          ...
          ══════════════════════════════════════════════════ */}
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-primary-700/10 shadow-2xl z-50 lg:hidden">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
          {biz.phone && (
            <a
              href={`tel:${biz.phone}`}
              className="btn-secondary rounded-btn flex-1 flex items-center justify-center gap-2 py-3 text-sm font-semibold transition-all duration-200 ease-advia"
            >
              <svg className="w-4 h-4 text-primary-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
              Call
            </a>
          ...
          <Link
            href={`/book/${slug}`}
            className="btn-primary rounded-btn flex-[2] flex items-center justify-center gap-2 py-3 text-sm font-semibold transition-all duration-200 ease-advia"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" /></svg>
            Book Now
          </Link>
